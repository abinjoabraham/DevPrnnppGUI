...
class PolygonModel(object):
    ...
    def _prediction(self):
        return {                                                                          ## Somekind of mapping also happening here as in dict
            'polys': self.graph.get_tensor_by_name(self.OUTPUT_POLYS_TENSOR_NAME),        ##'OutputPolys:0' Some scripts to load from the model
            'masks': self.graph.get_tensor_by_name(self.OUTPUT_MASKS_TENSOR_NAME),        ##'OutputMasks:0'
            'state1': self.graph.get_tensor_by_name(self.OUTPUT_STATE1_TENSOR_NAME),      ##'OutputState1:0'
            'state2': self.graph.get_tensor_by_name(self.OUTPUT_STATE2_TENSOR_NAME),      ##'OutputState2:0'
            'cnn_feats': self.graph.get_tensor_by_name(self.OUTPUT_CNN_FEATS_TENSOR_NAME) ##'OutputCNNFeats:0'
        }
    ...
    def do_test(self, sess, input_images, first_top_k=0):
        ...
        ## Predictions are over here
        pred_dict = sess.run(
            self._prediction(),                 ## Operations or assignments as the first argument in the sess.run and then place holders feed_dict
            feed_dict={self.INPUT_IMGS_TENSOR_NAME: input_images, self.INPUT_FIRST_TOP_K: first_top_k}      ## some kind of pointing over here
        )
        #
        ## pred_dict tensors
